...
max_iters = 1000000
block_size = 1000  # this is context length
learning_rate = 3e-04
...
def correct_ppg_outliers(valid_data, std_threshold=0.1):
    peaks, _ = scipy.signal.find_peaks(valid_data)
    systolic_values = valid_data[peaks]
    systolic_mean = np.mean(systolic_values)

    valleys, _ = scipy.signal.find_peaks(-valid_data)
    diastolic_values = valid_data[valleys]
    diastolic_mean = np.mean(diastolic_values)

    signal_mean = np.mean(valid_data)
    signal_std = np.std(valid_data)

    corrected_data = valid_data.copy()

    upper_threshold = signal_mean + std_threshold * signal_std
    lower_threshold = signal_mean - std_threshold * signal_std

    corrected_data[valid_data > upper_threshold] = systolic_mean
    corrected_data[valid_data < lower_threshold] = diastolic_mean

    return corrected_data


def visualize_ppg_points(signal):
    import matplotlib.pyplot as plt
    ...
    plt.plot(signal, 'k-', label='PPG Signal', alpha=0.7)
    plt.plot(peaks, signal[peaks], 'ro', label='Systolic Peaks')
    plt.plot(valleys, signal[valleys], 'bo', label='Diastolic Valleys')
    ...
    systolic_mean = np.mean(signal[peaks])
    diastolic_mean = np.mean(signal[valleys])

    plt.axhline(y=systolic_mean, color='r', linestyle='--', alpha=0.3,
                label=f'Systolic Mean: {systolic_mean:.2f}')
    plt.axhline(y=diastolic_mean, color='b', linestyle='--', alpha=0.3,
                label=f'Diastolic Mean: {diastolic_mean:.2f}')
    ...
    print(f"Average systolic value: {systolic_mean:.2f}")
    print(f"Average diastolic value: {diastolic_mean:.2f}")
...
def reshape_vector_to_matrix(vector, row_size=block_size):
    ...
    while i * stride + row_size <= n:
        ...
        try:
            ...
            visualize_ppg_points(valid_data)
            corrected_data = correct_ppg_outliers(valid_data, std_threshold=2)
            visualize_ppg_points(corrected_data)
            ...
            if (len(wd['peaklist']) - len(wd['removed_beats'])) > (block_size / 25) / 2:
                ...
                peak_index.extend(peaks)
                real_peaks = [item for item in peaks if item not in fake_peaks]
                sum_peaks = []
                for j in real_peaks:
                    sum_peaks.append(valid_data[j])
                sum_peaks = np.array(sum_peaks)
                peaks_mean = np.mean(sum_peaks)
                peaks_std = np.std(sum_peaks)

                for k in peak_index: # 빨간색 포함, 모든 피크
                    if(valid_data[k]>peaks_mean+peaks_std*var):
                        valid_data[k]=peaks_mean
                
                min_val = np.min(valid_data)
                max_val = np.max(valid_data)
                normalized_data = ((valid_data - min_val) / (max_val - min_val)) * 100
                plt.plot(normalized_data)
                plt.show()
                valid_segments.append(normalized_data[:row_size])
                '''
            min_val = np.min(valid_data)
            max_val = np.max(valid_data)
            normalized_data = ((valid_data - min_val) / (max_val - min_val)) * 100
            ...
            print(f"세그먼트 {i} process 함수 실행 안됨")
        i += 1
    ...
    if matrix.size > 0:
        np.savetxt(f"filtered_processed_blocksize_{block_size}_strides_{stride}_ppg_data.txt",
...
for iter in range(max_iters):
    ...
    if iter % save_interval == 0:
        ...
        model_path = f"C:/Users/user/PycharmProjects/Emotion Classification 3/HeartGPT-main/Model_files/PPGPT_pretrained_{n_embd}_{n_head}_{n_layer}_{block_size}_{max_iters}_{iter}.pth"
