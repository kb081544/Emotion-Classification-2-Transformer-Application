'''
확실한 1 짜리만 받아서 HeartGPT로 새로 생성 후,
0~100까지 나온 심박 데이터를 max min 값을 받아서 복원.

안정된 데이터와 차이나는 데이터를 확실히 다르게 생성하는지 부터 확인한 후,
따로 CNN이나 DNN 모델을 빌드해서 0과 1을 분류하는 모델을 만들어보자

이 파일에서 확실한 1 짜리 심박 데이터만 생성해보자
...
n_layer = 8
dropout = 0.2
model_path_ppg = r"C:\Users\user\PycharmProjects\Emotion Classification 3\HeartGPT-main\Model_files\PPGPT_500k_iters\PPGPT_500k_iters.pth"
